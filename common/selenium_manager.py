...
class SeleniumManager():
    ...
    def start_chrome(self):
        ...
        self.options.add_argument('--user-agent=' + USER_AGENT) #　リクエストヘッダ
    ...
    def select_element(self, name: str, select_text: str, mode: str = "", by: str = "NAME", index: int=0):
        '''
        Select要素から指定の名称に一致する選択肢を選択する
        '''
        if by == "NAME":
            select_element = self.chrome.find_elements_by_name(name)
        elif by == "ID":
            select_element = self.chrome.find_elements_by_id(name)
        else:
            select_element = self.chrome.find_elements_by_css_selector(name)
